...
const scenario: Scenario = {
  id: 'ppmob-01-login',
  name: 'ProjeXtPal Mobile - Login',
  app: 'projectpal_mobile',
  description: 'Authenticate to ProjeXtPal from a mobile context',
        ...
        for (const path of loginPaths) {
          ...
          if (res.ok) {
            ...
            if (token) {
              ...
              ctx.log(`ProjeXtPal Mobile login successful via ${path}`);
              return;
            }
          }
        }
        throw new Error('ProjeXtPal Mobile login failed on all paths');
      },
    },
    {
