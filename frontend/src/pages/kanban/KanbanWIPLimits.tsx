...
const KanbanWIPLimits = () => {
  ...
  const saveWip = async (colId: number) => { const val = wipEdits[colId]; try { const body: any = {}; if (val && parseInt(val) > 0) body.wip_limit = parseInt(val); else body.wip_limit = null; const r = await fetch(`/api/v1/projects/${id}/kanban/columns/${colId}/`, { method: "PATCH", headers: jsonHeaders, body: JSON.stringify(body) }); if (r.ok) { toast.success("WIP limit opgeslagen"); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } };
