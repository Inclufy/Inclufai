...
const AgileUserPersonas = () => {
  ...
  const [form, setForm] = useState({ name: "", role: "", goals: "", pain_points: "", behaviors: "" });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", role: "", goals: "", pain_points: "", behaviors: "" }); setDialogOpen(true); };
  const openEdit = (p: any) => { setEditing(p); setForm({ name: p.name || "", role: p.role || "", goals: p.goals || "", pain_points: p.pain_points || "", behaviors: p.behaviors || "" }); setDialogOpen(true); };
  const handleSave = async () => { if (!form.name) { toast.error("Naam verplicht"); return; } setSubmitting(true); try { const url = editing ? `/api/v1/projects/${id}/agile/personas/${editing.id}/` : `/api/v1/projects/${id}/agile/personas/`; const method = editing ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(form) }); if (r.ok) { toast.success("Opgeslagen"); setDialogOpen(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } finally { setSubmitting(false); } };
    ...
    <div className="min-h-full bg-background"><ProjectHeader />
        ...
        {personas.length === 0 ? <Card className="p-8 text-center"><Users className="h-12 w-12 mx-auto text-muted-foreground mb-4" /><h3 className="text-lg font-semibold">{pt("No personas yet")}</h3></Card> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{personas.map(p => (
            <Card key={p.id} className="group relative"><CardContent className="p-4">
              ...
              <div className="flex items-center gap-3 mb-3"><div className="h-12 w-12 rounded-full bg-pink-100 flex items-center justify-center"><span className="text-lg font-bold text-pink-600">{p.name?.charAt(0)}</span></div><div><p className="font-semibold">{p.name}</p>{p.role && <p className="text-xs text-muted-foreground">{p.role}</p>}</div></div>
              {p.goals && <div className="mb-2"><p className="text-xs font-semibold text-muted-foreground">🎯 Goals</p><p className="text-sm">{p.goals}</p></div>}
              {p.pain_points && <div className="mb-2"><p className="text-xs font-semibold text-muted-foreground">😤 Pain Points</p><p className="text-sm">{p.pain_points}</p></div>}
              {p.behaviors && <div><p className="text-xs font-semibold text-muted-foreground">💡 Behaviors</p><p className="text-sm">{p.behaviors}</p></div>}
            </CardContent></Card>
          ))}</div>
        )}
      </div>
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? pt("Edit") : pt("Add")} Persona</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4"><div className="space-y-2"><Label>{pt("Name")} *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div><div className="space-y-2"><Label>{pt("Role")}</Label><Input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div></div>
          <div className="space-y-2"><Label>Goals</Label><textarea className="w-full min-h-[60px] px-3 py-2 border rounded-md bg-background" value={form.goals} onChange={(e) => setForm({ ...form, goals: e.target.value })} /></div>
          <div className="space-y-2"><Label>Pain Points</Label><textarea className="w-full min-h-[60px] px-3 py-2 border rounded-md bg-background" value={form.pain_points} onChange={(e) => setForm({ ...form, pain_points: e.target.value })} /></div>
          <div className="space-y-2"><Label>Behaviors</Label><textarea className="w-full min-h-[60px] px-3 py-2 border rounded-md bg-background" value={form.behaviors} onChange={(e) => setForm({ ...form, behaviors: e.target.value })} /></div>
