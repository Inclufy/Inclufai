...
const AgileTeam = () => {
  ...
  const [form, setForm] = useState({ name: "", role: "developer", email: "", skills: "" });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", role: "developer", email: "", skills: "" }); setDialogOpen(true); };
  const openEdit = (m: any) => { setEditing(m); setForm({ name: m.name || "", role: m.role || "developer", email: m.email || "", skills: m.skills || "" }); setDialogOpen(true); };
  const handleSave = async () => { if (!form.name) { toast.error("Naam verplicht"); return; } setSubmitting(true); try { const url = editing ? `/api/v1/projects/${id}/agile/team/${editing.id}/` : `/api/v1/projects/${id}/agile/team/`; const method = editing ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(form) }); if (r.ok) { toast.success("Opgeslagen"); setDialogOpen(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } finally { setSubmitting(false); } };
  ...
  const roleColors: Record<string, string> = { product_owner: "bg-blue-100 text-blue-700", scrum_master: "bg-purple-100 text-purple-700", developer: "bg-green-100 text-green-700", designer: "bg-pink-100 text-pink-700", tester: "bg-amber-100 text-amber-700" };
    ...
    <div className="min-h-full bg-background"><ProjectHeader />
        ...
        {members.length === 0 ? <Card className="p-8 text-center"><Users className="h-12 w-12 mx-auto text-muted-foreground mb-4" /><h3 className="text-lg font-semibold">{pt("No team members yet")}</h3></Card> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{members.map(m => (
            <Card key={m.id} className="group relative"><CardContent className="p-4">
              ...
              <div className="flex items-center gap-3 mb-2"><div className="h-10 w-10 rounded-full bg-emerald-100 flex items-center justify-center"><span className="font-semibold text-emerald-600">{m.name?.charAt(0)?.toUpperCase()}</span></div><div><p className="font-medium">{m.name}</p>{m.email && <p className="text-xs text-muted-foreground">{m.email}</p>}</div></div>
              <Badge className={`text-xs ${roleColors[m.role] || "bg-gray-100 text-gray-700"}`}>{m.role?.replace("_", " ")}</Badge>
              {m.skills && <p className="text-xs text-muted-foreground mt-2">{m.skills}</p>}
            </CardContent></Card>
          ))}</div>
        )}
      </div>
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? pt("Edit") : pt("Add")} Member</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4"><div className="space-y-2"><Label>{pt("Name")} *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div><div className="space-y-2"><Label>{pt("Email")}</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div></div>
          <div className="space-y-2"><Label>{pt("Role")}</Label><Select value={form.role} onValueChange={(v) => setForm({ ...form, role: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="product_owner">Product Owner</SelectItem><SelectItem value="scrum_master">Scrum Master</SelectItem><SelectItem value="developer">Developer</SelectItem><SelectItem value="designer">Designer</SelectItem><SelectItem value="tester">Tester</SelectItem></SelectContent></Select></div>
          <div className="space-y-2"><Label>Skills</Label><Input value={form.skills} onChange={(e) => setForm({ ...form, skills: e.target.value })} /></div>
