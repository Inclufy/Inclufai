...
const WaterfallDevelopment = () => {
  ...
  const handleSave = async () => { if (!form.title) { toast.error("Titel verplicht"); return; } setSubmitting(true); try { const body: any = { ...form }; if (form.phase) body.phase = parseInt(form.phase); else delete body.phase; if (form.estimated_hours) body.estimated_hours = parseFloat(form.estimated_hours); else delete body.estimated_hours; const url = editing ? `/api/v1/projects/${id}/waterfall/tasks/${editing.id}/` : `/api/v1/projects/${id}/waterfall/tasks/`; const method = editing ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(body) }); if (r.ok) { toast.success("Opgeslagen"); setDialogOpen(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } finally { setSubmitting(false); } };
