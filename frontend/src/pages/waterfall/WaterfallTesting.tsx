...
const WaterfallTesting = () => {
  ...
  const handleSave = async () => { if (!form.title) { toast.error("Titel verplicht"); return; } setSubmitting(true); try { const url = editing ? `/api/v1/projects/${id}/waterfall/test-cases/${editing.id}/` : `/api/v1/projects/${id}/waterfall/test-cases/`; const method = editing ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(form) }); if (r.ok) { toast.success("Opgeslagen"); setDialogOpen(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } finally { setSubmitting(false); } };
  const handleDelete = async (tId: number) => { if (!confirm("Verwijderen?")) return; try { const r = await fetch(`/api/v1/projects/${id}/waterfall/test-cases/${tId}/`, { method: "DELETE", headers }); if (r.ok || r.status === 204) { toast.success("Verwijderd"); fetchData(); } } catch { toast.error("Verwijderen mislukt"); } };
  const handleExecute = async (tId: number, result: string) => { try { const r = await fetch(`/api/v1/projects/${id}/waterfall/test-cases/${tId}/execute/`, { method: "POST", headers: jsonHeaders, body: JSON.stringify({ result }) }); if (r.ok) { toast.success("Test uitgevoerd"); fetchData(); } } catch { toast.error("Uitvoeren mislukt"); } };
