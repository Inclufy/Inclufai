...
const WaterfallDeployment = () => {
  ...
  const initDefaults = async () => { try { const r = await fetch(`/api/v1/projects/${id}/waterfall/deployment/initialize/`, { method: "POST", headers: jsonHeaders }); if (r.ok) { toast.success("Checklist geïnitialiseerd"); fetchData(); } else toast.error("Initialiseren mislukt"); } catch { toast.error("Initialiseren mislukt"); } };
  const handleToggle = async (dId: number) => { try { const r = await fetch(`/api/v1/projects/${id}/waterfall/deployment/${dId}/toggle/`, { method: "POST", headers: jsonHeaders }); if (r.ok) fetchData(); } catch { toast.error("Toggle mislukt"); } };
  const handleCreate = async () => { if (!form.title) { toast.error("Titel verplicht"); return; } setSubmitting(true); try { const r = await fetch(`/api/v1/projects/${id}/waterfall/deployment/`, { method: "POST", headers: jsonHeaders, body: JSON.stringify(form) }); if (r.ok) { toast.success("Toegevoegd"); setDialogOpen(false); fetchData(); } else toast.error("Toevoegen mislukt"); } catch { toast.error("Toevoegen mislukt"); } finally { setSubmitting(false); } };
  const handleDelete = async (dId: number) => { if (!confirm("Verwijderen?")) return; try { const r = await fetch(`/api/v1/projects/${id}/waterfall/deployment/${dId}/`, { method: "DELETE", headers }); if (r.ok || r.status === 204) { toast.success("Verwijderd"); fetchData(); } } catch { toast.error("Verwijderen mislukt"); } };
