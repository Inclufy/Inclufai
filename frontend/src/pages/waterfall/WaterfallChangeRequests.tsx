...
const WaterfallChangeRequests = () => {
  ...
  const handleSave = async () => { if (!form.title) { toast.error("Titel verplicht"); return; } setSubmitting(true); try { const url = editing ? `/api/v1/projects/${id}/waterfall/change-requests/${editing.id}/` : `/api/v1/projects/${id}/waterfall/change-requests/`; const method = editing ? "PATCH" : "POST"; const r = await fetch(url, { method, headers: jsonHeaders, body: JSON.stringify(form) }); if (r.ok) { toast.success("Opgeslagen"); setDialogOpen(false); fetchData(); } else toast.error("Opslaan mislukt"); } catch { toast.error("Opslaan mislukt"); } finally { setSubmitting(false); } };
  const handleDelete = async (cId: number) => { if (!confirm("Verwijderen?")) return; try { const r = await fetch(`/api/v1/projects/${id}/waterfall/change-requests/${cId}/`, { method: "DELETE", headers }); if (r.ok || r.status === 204) { toast.success("Verwijderd"); fetchData(); } } catch { toast.error("Verwijderen mislukt"); } };
  const handleAction = async (cId: number, action: string) => { try { const r = await fetch(`/api/v1/projects/${id}/waterfall/change-requests/${cId}/${action}/`, { method: "POST", headers: jsonHeaders }); if (r.ok) { toast.success("Actie uitgevoerd"); fetchData(); } else toast.error("Actie mislukt"); } catch { toast.error("Actie mislukt"); } };
