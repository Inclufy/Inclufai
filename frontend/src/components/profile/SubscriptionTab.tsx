  ...
  const plans: DisplayPlan[] = (plansData?.results || plansData || [])
    ...
    .map((p: BackendPlan) => {
      ...
      if (maxProjects === null || maxProjects === 0 || maxProjects === -1) {
        features.push({ included: true, text: 'Unlimited Projects' });
      } else {
        features.push({ included: true, text: `${maxProjects} Projects` });
      }
      
      // Users - Handle null as unlimited
      const maxUsers = p.max_users;
      if (maxUsers === null || maxUsers === 0 || maxUsers === -1) {
        features.push({ included: true, text: 'Unlimited Users' });
      } else if (maxUsers === 1) {
        features.push({ included: true, text: '1 User' });
      } else {
        features.push({ included: true, text: `Up to ${maxUsers} Users` });
      }
      ...
      if (hasFeature('tijdregistratie') || hasFeature('time')) {
        features.push({ included: true, text: 'Time Tracking' });
      }
      if (hasFeature('ai')) {
        features.push({ included: true, text: 'AI Assistant' });
      }
      if (hasFeature('programma') || hasFeature('program')) {
        features.push({ included: true, text: 'Program Management' });
      }
      if (hasFeature('team') || hasFeature('collaboration')) {
        features.push({ included: true, text: 'Team Collaboration' });
      }
      if (p.priority_support) {
        features.push({ included: true, text: 'Priority Support' });
      }
      if (p.advanced_analytics) {
        features.push({ included: true, text: 'Advanced Analytics' });
      }
      if (hasFeature('gantt')) {
        features.push({ included: true, text: 'Gantt Charts' });
      }
      
      // Add negative features for lower tiers
      if (planLevel === 'starter') {
        features.push({ included: false, text: 'Advanced Analytics' });
        features.push({ included: false, text: 'Team Features' });
        features.push({ included: false, text: 'Program Management' });
      }
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  {isTrial ? 'Limited trial features' : 'Full access to all features'}
                </p>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-800 dark:text-amber-300">
                      Upgrade to unlock unlimited projects, advanced features, and team collaboration
                    </p>
                  </div>
                </div>
              </div>
      ...
      {(isTrial || tier === 'starter') && (
        <div>
          ...
          {plansLoading ? (
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>No plans available at the moment. Please contact support.</AlertDescription>
            </Alert>
          ) : (
